...
trait ModxZip
{
    ...
    {
        if ($version === 'latest') {
            $this->output->writeln('Looking up latest MODX version...'. $version);
            $link = $this->fetchUrl($version);
            $version = basename($link, '.zip');

            // When this happens: https://modx.com/download/direct?id=5a340eb0bc8dd3550a8b4567.zip
            if (str_replace('?', '', $version) !== $version) {
                $this->output->writeln('<comment>Could not download MODX version returned: ' . $version . '</comment>');
                return false;
            }
    ...
    protected function download($version)
    {
        ...
        if (!file_exists(BLEND_CACHE_DIR)) {
            mkdir(BLEND_CACHE_DIR);
        }

        $this->removeOutdatedArchive($version); // remove old files

        $zip = BLEND_CACHE_DIR . $version . '.zip';
    ...
    {
        $folder = BLEND_CACHE_DIR . $version;
        $package = $folder . '.zip';
